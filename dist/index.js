...
export default {
    ...
    async register(api) {
        ...
        const debounceMs = typeof config.debounceMs === "number" ? config.debounceMs : 3000;
        ...
        try {
            ...
            switch (state) {
                ...
                case "clawvc-owned":
                    ...
                    break;
                case "user-owned":
                    console.warn(`[clawvc] Workspace has existing user git history — refusing to start. ` +
        ...
        if (autoWatch) {
            try {
                ...
                console.error(`[clawvc] Failed to start watcher: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        // --- Register tools ---
        api.registerTool({
            name: "clawvc_log",
            description: "Show recent workspace change history. Each entry is an auto-committed snapshot from a previous agent turn.",
            parameters: Type.Object({
                count: Type.Optional(Type.Integer({ description: "Number of commits to show", default: 20 })),
            }),
        ...
        api.registerTool({
            name: "clawvc_diff",
            description: "Show what changed N commits ago. Defaults to the most recent change.",
            parameters: Type.Object({
                n: Type.Optional(Type.Integer({ description: "Commits ago (default 1)", default: 1 })),
            }),
            async execute(_id, params) {
                try {
                    const n = params.n ?? 1;
        ...
        api.registerTool({
            name: "clawvc_show",
            description: "Show the full diff of the Nth last change, including file stats and patch.",
            parameters: Type.Object({
                n: Type.Optional(Type.Integer({ description: "Commits ago (default 1)", default: 1 })),
            }),
            async execute(_id, params) {
        ...
        api.registerTool({
            name: "clawvc_undo",
            description: "Undo the Nth last workspace change using git revert. " +
        ...
        api.registerTool({
            name: "clawvc_status",
            description: "Show clawvc status: whether the file watcher is running, plan mode state, and the last commit.",
            parameters: Type.Object({}),
            async execute() {
                try {
        ...
        api.registerTool({
            name: "clawvc_plan",
            description: "Toggle plan mode. When ON, the agent becomes read-only — it can read files and reason " +
                "but cannot write, edit, or execute commands. Use this to safely ask the agent for a plan " +
                "before letting it execute.",
            parameters: Type.Object({
                action: Type.Union([Type.Literal("on"), Type.Literal("off"), Type.Literal("status")], { description: 'Action: "on", "off", or "status"' }),
            }),
            async execute(_id, params) {
                try {
                    ...
                    switch (action) {
                        ...
                        case "status": {
                            ...
                            return textResult(result.active
